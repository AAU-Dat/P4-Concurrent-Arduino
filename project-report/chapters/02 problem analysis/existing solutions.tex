...
\todo{From Emphasis to end needs to be reformulated}
...
\todo{Bastardized i belive is a negative ladet ord}
...
Lastly, when using protothreads, some of the Arduino functions - like delay() - should not be used. Protothreads are already blocking if \todo{written to be, sounds strange} written to be, so blocking the cpu with delay() could potentially prevent protothreads from executing correctly~\cite{AdamDunkelProtothreads}.
