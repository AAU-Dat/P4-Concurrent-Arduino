...
In Figure~\ref{fig:arccompilermodel}, the source code is first scanned into tokens and then parsed into an \gls{ast}, Antlr handles all of this from our grammar, hence the box around the scanner and parser. The \gls{ast} is then type checked and scope checked. The type and scope check rules is explained in more detail in section 3.4. Once the transpiler has done all of the neccessary checks on the source code, it is ready to generate the Arduino code, which is also the final step of the compiler. We should now have a target code, ready for the Arduino, based from our source Arc code.